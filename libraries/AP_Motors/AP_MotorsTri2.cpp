...
/*
 *       AP_MotorsTri2.cpp - ArduCopter motors library
 *       Code by RandyMackay. DIYDrones.com
 *
 */
#include <AP_HAL.h>
#include <AP_Math.h>
#include "AP_MotorsTri2.h"

extern const AP_HAL::HAL& hal;

// init
void AP_MotorsTri2::Init()
{
...
void AP_MotorsTri2::set_update_rate( uint16_t speed_hz )
{
...
void AP_MotorsTri2::enable()
{
    ...
    hal.rcout->enable_ch(AP_MOTORS_CH_TRI2_YAWR);
    hal.rcout->enable_ch(AP_MOTORS_CH_TRI2_YAWL);
}

// output_min - sends minimum values out to the motors
void AP_MotorsTri2::output_min()
{
    ...
    hal.rcout->write(_motor_to_channel_map[AP_MOTORS_CH_TRI2_YAWR], _rc_yaw->radio_trim);
    hal.rcout->write(_motor_to_channel_map[AP_MOTORS_CH_TRI2_YAWL], _rc_yaw->radio_trim);
}

// output_armed - sends commands to the motors
void AP_MotorsTri2::output_armed()
{
    ...
    if( _rc_yawr->get_reverse() == true ) {
        hal.rcout->write(AP_MOTORS_CH_TRI2_YAWR, _rc_yawr->radio_trim - (_rc_yawr->radio_out - _rc_yawr->radio_trim));
    }else{
        hal.rcout->write(AP_MOTORS_CH_TRI2_YAWR, _rc_yawr->radio_out);
    }
    
    if( _rc_yawl->get_reverse() == true ) {
        hal.rcout->write(AP_MOTORS_CH_TRI2_YAWL, _rc_yawl->radio_trim - (_rc_yawl->radio_out - _rc_yawl->radio_trim));
    }else{
        hal.rcout->write(AP_MOTORS_CH_TRI2_YAWL, _rc_yawl->radio_out);
    }
}

// output_disarmed - sends commands to the motors
void AP_MotorsTri2::output_disarmed()
{
...
void AP_MotorsTri2::output_test()
{
    // Send minimum values to all motors
    output_min();
